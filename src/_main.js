...
const segoe_ui = new Font("/assets/font/Segoe UI.ttf");
...
const sound_table = System.listDir("/assets/sounds").map(file => file.name).filter(str => str.endsWith(".ogg") || str.endsWith(".wav") );

console.log(JSON.stringify(System.listDir("/assets/sounds")));
...
let track = Sound.load("/assets/sounds/" + sounds[0].file);
...
const icons = {play:new Image("/assets/amp/play.png"), pause:new Image("/assets/amp/pause.png"), next:new Image("/assets/amp/next.png"), back:new Image("/assets/amp/back.png")};
...
while(true){
    ...
    if(pad.justPressed(Pads.L1)) {
        ...
        let new_track = Sound.load("/assets/sounds/" + sounds[0].file);
